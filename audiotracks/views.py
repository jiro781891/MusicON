...
def user_index(request, username, page_number=None):
    ...
    page, tracks = paginate(tracks, page_number)
    base_path = reverse('user_index', args=[username])
...
def upload_track(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('edit_track',
                                                             args=[track.id]))
    else:
...
def edit_track(request, track_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            redirect_url = reverse('user_index', args=[username])
            return HttpResponseRedirect(redirect_url)
    else:
        form = TrackEditForm(instance=track, )
    track_url_args = ['']
    track_url_args.insert(0, username)
    track_detail_url = reverse('track_detail',
                                            args=track_url_args)
...
def confirm_delete_track(request, track_id):
    ...
    default_origin_url = reverse('user_index',
